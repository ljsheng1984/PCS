...
{
    public partial class dhau : CheckLoginPage
    {
        ...
        protected void tijiao_Click(object sender, EventArgs e)
        {
            ...
            {
                Guid gid = Guid.Parse(Request.QueryString["gid"]);
                ef = db.dhmb.Where(l => l.gid == gid).FirstOrDefault();
            }
            ef.dianhua = dianhua.Value;
            ...
            if (db.SaveChanges() == 1)
            {
                db.zdlb.Where(l => l.jian == "dhmb").Update(l => new zdlb { zhi = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") });
