<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="mclist.aspx.cs" Inherits="LJSheng.Web.ljsheng.lin.mclist" %>
<!DOCTYPE html>
    ...
    <form id="form1" runat="server">
        <div class="cont">
            <div class="cont2">
                <div class="opera">
                    <p>
                        <img src="/ljsheng/images/16.png" /> 显示名称<input type="text" id="xsmc" runat="server" class="txt1" /> 电话号码：<input type="text" id="haoma" runat="server" class="txt1" />
                        状态:<asp:DropDownList runat="server" ID="xsDDL">
                            <asp:ListItem Text="= 全 部 =" Value="0"></asp:ListItem>
                            <asp:ListItem Text="不拦截" Value="1"></asp:ListItem>
                            <asp:ListItem Text="拦截" Value="2"></asp:ListItem>
                            </asp:DropDownList>
                ...
                <div class="content">
                    <asp:ListView ID="LVljsheng" runat="server" OnItemCommand="LVljsheng_ItemCommand">
                        <ItemTemplate>
                            <tr>
                                <td><%# Eval("haoma")%><asp:Label ID="gid" runat="server" Text='<%# Eval("gid") %>' Visible="false" /></td>
                                <td><%# Eval("xsmc")%></td>
                                <td>
                                    <asp:Button ID="xs" runat="server" CssClass="btn1" Text='<%# Eval("xs").ToString() == "1" ? "不拦截" : "拦截"%>' CommandName="xs" CommandArgument='<%# Eval("xs") %>' />
                                </td>
                                <td><%# DateTime.Parse(Eval("rukusj").ToString()).ToString("yyyy-MM-dd HH:mm")%></td>
                                <td><a href="javascript:showiframe('<%# Eval("haoma")%>','mcau.aspx?gid=<%#Eval("gid") %>')">修改</a> | <asp:Button ID="del" runat="server" Text="删除" CssClass="btn2" CommandName="del" OnClientClick="return confirm('确定删除吗？');" /></td>
                            </tr>
                        </ItemTemplate>
                        <EmptyDataTemplate>
                            <div>当前没有符合条件的数据</div>
