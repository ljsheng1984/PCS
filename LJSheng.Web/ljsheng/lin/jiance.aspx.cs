...
{
    public partial class jiance : CheckLoginPage
    {
        ...
        private void Bind()
        {
            LVljsheng.DataSource = Data.Tables.Table_List("jiance", "zt,rukusj DESC", "*", 88888, 1, ViewState["where"].ToString());
            LVljsheng.DataBind();
        }
        ...
        protected void LVljsheng_ItemCommand(object sender, ListViewCommandEventArgs e)
        {
            ...
            {
                case "del":
                    if (db.jiance.Where(l => l.gid == gid).Delete() != 1)
                    {
