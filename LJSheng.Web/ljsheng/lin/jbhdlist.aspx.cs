...
{
    public partial class jbhdlist : CheckLoginPage
    {
        ...
        private void Bind()
        {
            LVljsheng.DataSource = Data.Tables.Table_List("View_jubao", "hdzt,rukusj DESC", "*", 88888, 1, ViewState["where"].ToString());
            LVljsheng.DataBind();
        }
        ...
        protected void LVljsheng_ItemCommand(object sender, ListViewCommandEventArgs e)
        {
            ...
            {
                case "del":
                    if (db.jubao.Where(l => l.gid == gid).Delete() != 1)
                    {
                ...
                case "hdzt":
                    ...
                    else
                    {
                        ...
                        Data.EF.huodong ef = db.huodong.Where(l => l.lx==1 && l.hygid == hygid && l.zt == 1).FirstOrDefault();
                        if (ef == null)
                        {
                        ...
                        if (db.SaveChanges() == 1)
                        {
                            LJSheng.Common.JS.Alert("增加用户举报次数成功", this);
                        }
                        else
                        {
                            LJSheng.Common.JS.Alert("增加用户举报次数失败", this);
                        }
                    }
                    break;
                default:
