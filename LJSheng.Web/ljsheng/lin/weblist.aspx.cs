...
{
    public partial class weblist : CheckLoginPage
    {
        ...
        private void Bind()
        {
            LVljsheng.DataSource = Data.Tables.Table_List("webmb", "rukusj DESC", "*", 88888, 1, ViewState["where"].ToString());
            LVljsheng.DataBind();
        }
        ...
        protected void LVljsheng_ItemCommand(object sender, ListViewCommandEventArgs e)
        {
            ...
            {
                case "del":
                    if (db.webmb.Where(l => l.gid == gid).Delete() != 1)
                    {
        ...
        protected void sel_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(url.Value.Trim()))
            {
                sb.Append(" AND url like '%" + url.Value.Trim() + "%'");
            }
