<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="dhlist.aspx.cs" Inherits="LJSheng.Web.ljsheng.lin.dhlist" %>
<!DOCTYPE html>
    ...
    <form id="form1" runat="server">
        <div class="cont">
            <div class="cont2">
                <div class="opera">
                    <p>
                        <img src="/ljsheng/images/16.png" /> 电话号码：<input type="text" id="dianhua" runat="server" class="txt1" />
                        状态:<asp:DropDownList runat="server" ID="ztDDL">
                            <asp:ListItem Text="= 全 部 =" Value="0"></asp:ListItem>
                            <asp:ListItem Text="启用" Value="1"></asp:ListItem>
                            <asp:ListItem Text="未启用" Value="2"></asp:ListItem>
                            </asp:DropDownList>
                ...
                <div class="content">
                    <asp:ListView ID="LVljsheng" runat="server" OnItemCommand="LVljsheng_ItemCommand">
                        <ItemTemplate>
                            <tr>
                                <td><%# Eval("dianhua")%><asp:Label ID="gid" runat="server" Text='<%# Eval("gid") %>' Visible="false" /></td>
                                <td>
                                    <asp:Button ID="zt" runat="server" CssClass="btn1" Text='<%# Eval("zt").ToString() == "1" ? "启用" : "不启用"%>' CommandName="zt" CommandArgument='<%# Eval("zt") %>' />
                                </td>
                                <td><%# DateTime.Parse(Eval("rukusj").ToString()).ToString("yyyy-MM-dd HH:mm")%></td>
                                <td><a href="javascript:showiframe('<%# Eval("dianhua")%>','dhau.aspx?gid=<%#Eval("gid") %>')">修改</a> | <asp:Button ID="del" runat="server" Text="删除" CssClass="btn2" CommandName="del" OnClientClick="return confirm('确定删除吗？');" /></td>
                            </tr>
                        </ItemTemplate>
                        <EmptyDataTemplate>
                            <div>当前没有符合条件的数据</div>
