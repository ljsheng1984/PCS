...
{
    public partial class mclist : CheckLoginPage
    {
        ...
        private void Bind()
        {
            LVljsheng.DataSource = Data.Tables.Table_List("dianhua", "rukusj DESC", "*", 88888, 1, ViewState["where"].ToString());
            LVljsheng.DataBind();
        }
        ...
        protected void LVljsheng_ItemCommand(object sender, ListViewCommandEventArgs e)
        {
            ...
            {
                case "del":
                    if (db.dianhua.Where(l => l.gid == gid).Delete() != 1)
                    {
        ...
        protected void sel_Click(object sender, EventArgs e)
        {
            ...
            {
                sb.Append(" AND haoma like '%" + haoma.Value.Trim() + "%'");
            }
            if (!string.IsNullOrEmpty(xsmc.Value.Trim()))
            {
                sb.Append(" AND xsmc like '%" + xsmc.Value.Trim() + "%'");
            }
            if (xsDDL.SelectedValue != "0")
            {
                sb.Append(" AND xs = " + xsDDL.SelectedValue);
            }
            //根据开始时间和结束时间查询
