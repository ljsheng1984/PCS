<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="xinwenlist.aspx.cs" Inherits="LJSheng.Web.ljsheng.lin.xinwenlist" %>
    ...
    <form id="form1" runat="server">
        <div class="cont">
            <div class="cont2">
                <div class="opera">
                    <p>
                        <img src="/ljsheng/images/16.png" /> 标题：<input type="text" id="biaoti" runat="server" class="txt1" />
                ...
                <div class="content">
                    <asp:ListView ID="LVljsheng" runat="server" OnItemCommand="LVljsheng_ItemCommand">
                        <ItemTemplate>
                            <tr>
                                <td><a target="_blank" href="<%# string.IsNullOrEmpty(Eval("url").ToString()) ?"/app/xw.aspx?gid=" + Eval("gid").ToString(): Eval("url").ToString() %>"><%# Eval("biaoti")%></a><asp:Label ID="gid" runat="server" Text='<%# Eval("gid") %>' Visible="false" /></td>
                                <td><%# Eval("fubiao")%></td>
                                <td><a target="_blank" href="<%# LJSheng.Data.Helps.xinwen + Eval("tupian") %>"><img  width="50" height="50" src="<%# LJSheng.Data.Helps.xinwen + Eval("tupian") %>" onerror="this.src='/ljsheng/images/nopic.png'" /></a></td>
                                <td><%# Eval("px")%></td>
                                <td>
                                    <asp:Button ID="xs" runat="server" CssClass="btn1" Text='<%# Eval("xs").ToString() == "1" ? "显示" : "不显示"%>' CommandName="xs" CommandArgument='<%# Eval("xs") %>' />
                                </td>
                                <td><%# DateTime.Parse(Eval("rukusj").ToString()).ToString("yyyy-MM-dd HH:mm")%></td>
                                <td><a href="javascript:showiframe('<%# Eval("biaoti")%>','xinwenau.aspx?gid=<%#Eval("gid") %>')">修改</a> | <asp:Button ID="del" runat="server" Text="删除" CssClass="btn2" CommandName="del" OnClientClick="return confirm('确定删除吗？');" /></td>
                            </tr>
                        </ItemTemplate>
                        <EmptyDataTemplate>
                            <div>当前没有符合条件的数据</div>
