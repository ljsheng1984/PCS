...
{
    public partial class dhlist : CheckLoginPage
    {
        ...
        private void Bind()
        {
            LVljsheng.DataSource = Data.Tables.Table_List("dhmb", "rukusj DESC", "*", 88888, 1, ViewState["where"].ToString());
            LVljsheng.DataBind();
        }
        ...
        protected void LVljsheng_ItemCommand(object sender, ListViewCommandEventArgs e)
        {
            ...
            var b = db.dhmb.Where(l => l.gid == gid).FirstOrDefault();
            switch (e.CommandName)
            {
                case "del":
                    if (db.dhmb.Where(l => l.gid == gid).Delete() != 1)
                    {
        ...
        protected void sel_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(dianhua.Value.Trim()))
            {
                sb.Append(" AND dianhua like '%" + dianhua.Value.Trim() + "%'");
            }
