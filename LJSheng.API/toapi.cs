...
namespace LJSheng.API
{
    ...
    public static class toapi
    {
        ...
        /// 调用函数,反射接口
        /// </summary>
        /// <param name="ffm">方法名称</param>
        /// <param name="param">参数列表</param>
        /// <returns></returns>
        public static ApiResult InvokeFunction(string ffm, object[] param)
        {
            object data = (typeof(goapi).InvokeMember(ffm, BindingFlags.InvokeMethod | BindingFlags.Public | BindingFlags.Static | BindingFlags.OptionalParamBinding, null, null, param));
        ...
        /// <returns></returns>
        public static string ProcessRequest(string ffm,string ip, string param)
        {
            //接口运行时间统计
            ...
            try
            {
                #region 参数判断
                //判断调用的方法是否为空
                if (String.IsNullOrEmpty(ffm))
                {
                    throw (new ApiException(new ApiResult(ApiResultCodeEnum.ParameterError, "方法名不能为空",ffm)));
                }

                //判断调用的方法是否存在
                MethodInfo methodInfo = typeof(goapi).GetMethod(ffm);
                if (methodInfo==null)
                {
                    throw (new ApiException(new ApiResult(ApiResultCodeEnum.ParameterError, "方法名不存在", ffm)));
                }
                //判断param是否为空
                ...
                try
                {
                    ...
                    #endregion
                    //调用函数
                    ApiResult apiResult = InvokeFunction(ffm.ToLower(), paramList.ToArray());
                    //返回加密的程序
                    ...
                    //LJSheng.Common.LogManager.WriteLog(ffm, param);
        ...
        #region 异常统计
        private static void Bug(Exception e,string ffm,string sourceParam,string DESKey)
        {
            EFDB db = new EFDB();
